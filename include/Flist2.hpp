...
class Flist {
    ...
    class Link {
        ...
        //Move [beforeFirst+1, End) to after this
        void splice_after(Link* beforeFirst, Link* atLast) {
...
#pragma region Modifiers
    ...
    //[beforeFirst+1, End) is inserted (moved) to after this

    void splice_after(iterator pos, Flist& other, iterator beforeFirst, iterator End) {
        pos._ptr->splice_after(beforeFirst._ptr, End._ptr);
    }
...
#pragma region nonmembers
    ...
    friend std::ostream& operator<<(std::ostream& cout, const Flist& l) {
...
template<class T>
void swap(Flist<T>& lhs, Flist<T>& rhs) {
    lhs.swap(rhs);
}
